...
#define INTREPID2_TEST_CHECK_POINT_INCLUSION(offset, expected, shtopo, celltag) \
    ...
    struct MapPoints<shards::Tetrahedron<10>::key> {
      double operator()(const double* coords, int comp) {
        ...
        const auto& x = coords[0]; const auto& y = coords[1]; const auto& z = coords[2];
        return linearMap(coords, comp)-0.1*x*x -0.2*y*y +0.3*z*z +0.05*x*y +0.07*x*z -0.06*y*z;  //for simplicity we choose the same quadratic term for all components 
      }
    };
    ...
    struct MapPoints<shards::Wedge<15>::key> {
      double operator()(const double* coords, int comp) {
        ...
        const auto& x = coords[0]; const auto& y = coords[1]; const auto& z = coords[2];
        return wedge6Map(coords, comp)-0.05*(x*x - y*y + z*z +  x*y + x*y*z + y*z*z + x*x*z + y*y*z + x*z*z);  //for simplicity we choose the same quadratic term for all components 
      }
    };
    ...
    struct MapPoints<shards::Wedge<18>::key> {
      double operator()(const double* coords, int comp) {
        ...
        const auto& x = coords[0]; const auto& y = coords[1]; const auto& z = coords[2];
        return wedge15Map(coords, comp)+0.04*(x*x*z*z -  x*y*z*z + y*y*z*z);  //for simplicity we choose the same quadratic term for all components 
      }
    };
    ...
    struct MapPoints<shards::Hexahedron<20>::key> {
      double operator()(const double* coords, int comp) {
        ...
        const auto& x = coords[0]; const auto& y = coords[1]; const auto& z = coords[2];
        return trilinearMap(coords, comp)-0.05*(x*x - y*y + z*z +  x*y*z*(x-y-z)+ x*x*y + y*z*z + x*x*z + y*y*z+ x*y*y + x*z*z);  //for simplicity we choose the same quadratic term for all components 
      }
    };
    ...
    struct MapPoints<shards::Hexahedron<27>::key> {
      double operator()(const double* coords, int comp) {
        ...
        const auto& x = coords[0]; const auto& y = coords[1]; const auto& z = coords[2];
        return hex20Map(coords, comp)+0.07*(x*x*y*y + x*x*z*z + y*y*z*z + x*x*y*y*z*z   + x*y*z*(x*y-x*z+y*z));  //for simplicity we choose the same quadratic term for all components 
      }
    };
...
#define INTREPID2_COMPUTE_POINTS_AND_CELL_NODES_IN_PHYS_SPACE(inCell, host_points, physPoints, physNodes, shtopo, cellTools) \
    {                                                                                                              \
        ...
        switch (topo.getKey()) {                                                                                   \
          case shards::Line<2>::key:          basisPtr = Teuchos::rcp(new Basis_HGRAD_LINE_C1_FEM   <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Line<3>::key:          basisPtr = Teuchos::rcp(new Basis_HGRAD_LINE_C2_FEM   <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Triangle<3>::key:      basisPtr = Teuchos::rcp(new Basis_HGRAD_TRI_C1_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Quadrilateral<4>::key: basisPtr = Teuchos::rcp(new Basis_HGRAD_QUAD_C1_FEM   <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Tetrahedron<4>::key:   basisPtr = Teuchos::rcp(new Basis_HGRAD_TET_C1_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Hexahedron<8>::key:    basisPtr = Teuchos::rcp(new Basis_HGRAD_HEX_C1_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Wedge<6>::key:         basisPtr = Teuchos::rcp(new Basis_HGRAD_WEDGE_C1_FEM  <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Pyramid<5>::key:       basisPtr = Teuchos::rcp(new Basis_HGRAD_PYR_C1_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Triangle<6>::key:      basisPtr = Teuchos::rcp(new Basis_HGRAD_TRI_C2_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Quadrilateral<8>::key: basisPtr = Teuchos::rcp(new Basis_HGRAD_QUAD_I2_FEM   <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Quadrilateral<9>::key: basisPtr = Teuchos::rcp(new Basis_HGRAD_QUAD_C2_FEM   <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Tetrahedron<10>::key:  basisPtr = Teuchos::rcp(new Basis_HGRAD_TET_C2_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Tetrahedron<11>::key:  basisPtr = Teuchos::rcp(new Basis_HGRAD_TET_COMP12_FEM<Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Hexahedron<20>::key:   basisPtr = Teuchos::rcp(new Basis_HGRAD_HEX_I2_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Hexahedron<27>::key:   basisPtr = Teuchos::rcp(new Basis_HGRAD_HEX_C2_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Wedge<15>::key:        basisPtr = Teuchos::rcp(new Basis_HGRAD_WEDGE_I2_FEM  <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Wedge<18>::key:        basisPtr = Teuchos::rcp(new Basis_HGRAD_WEDGE_C2_FEM  <Kokkos::HostSpace::device_type, double, double>()); break;  \
          case shards::Pyramid<13>::key:      basisPtr = Teuchos::rcp(new Basis_HGRAD_PYR_I2_FEM    <Kokkos::HostSpace::device_type, double, double>()); break;  \
          default:   {};                                                                                                                                      \
        }                                                                                                                                                  \
                                                                                                                            \
                                            \
        ...
        physPoints = Kokkos::DynRankView<ValueType,DeviceType>("physPoints",1,host_points.extent(1),dim);             \
        ...
        for(size_t i=0; i< host_points.extent(1);++i) {                                                                \
          for (size_t d=0; d<dim; ++d)                                                                             \
            coords[d]= host_points(0,i,d);                                                                            \
          for (size_t d=0; d<dim; ++d)                                                                             \
            physPoints_h(0,i,d) = mapPoints(coords,d);                                                                  \
        }                                                                                                          \
...
#define INTREPID2_TEST_CHECK_POINTWISE_INCLUSION(inCell, points, physNodes, shtopo, cellTools)     \
    {                                                                                   \
        ...
        if(physNodes.extent(0)==0)                                                      \
          cellTools::checkPointwiseInclusion(Kokkos::subview(inCell,0,Kokkos::ALL()), Kokkos::subview(points,0,Kokkos::ALL(),Kokkos::ALL()), topo );                    \
        else {                                                                           \
      ...
      *outStream
      ...
      try {
        ...
        *outStream
        ...
        Kokkos::DynRankView<ValueType,DeviceType> pts1d("pts1d", 1, 4, 1);
        Kokkos::DynRankView<ValueType,DeviceType> pts2d("pts2d", 1, 4, 2);
        Kokkos::DynRankView<ValueType,DeviceType> pts3d("pts3d", 1, 4, 3);
        ...
        pts1d_h(0,0,0) = -1.0+eps;   //point near vertex (in)
        pts1d_h(0,1,0) = -1.0-eps;   //point near vertex (out)
        pts1d_h(0,2,0) = 1.0-eps;    //point near vertex (in)
        pts1d_h(0,3,0) = 1.0+eps;    //point near vertex (out)
        ...
        pts2d_h(0,0,0) = 0.0+eps; pts2d_h(0,0,1) = 0.0+eps; //point near vertex (in)
        pts2d_h(0,1,0) = 0.0-eps; pts2d_h(0,1,1) = 0.0-eps; //point near vertex (out)
        pts2d_h(0,2,0) = 0.5-eps; pts2d_h(0,2,1) = 0.5-eps; //point near edge (in)
        pts2d_h(0,3,0) = 0.5+eps; pts2d_h(0,3,1) = 0.5+eps; //point near edge (out)
        ...
        pts2d_h(0,0,0) = -1.0+eps; pts2d_h(0,0,1) = -1.0+eps;  //point near vertex (in)
        pts2d_h(0,1,0) = -1.0-eps; pts2d_h(0,1,1) = -1.0-eps;  //point near vertex (out)
        pts2d_h(0,2,0) = 0.0; pts2d_h(0,2,1) = -1.0+eps;       //point near edge (in)
        pts2d_h(0,3,0) = 0.0; pts2d_h(0,3,1) = -1.0-eps;       //point near edge (out)
        Kokkos::deep_copy(pts2d,pts2d_h);
        ...
        pts3d_h(0,0,0) = 0.0+eps; pts3d_h(0,0,1) = 0.0+eps; pts3d_h(0,0,2) = 0.0+eps;        //point near vertex (in)
        pts3d_h(0,1,0) = 0.0-eps; pts3d_h(0,1,1) = 0.0-eps; pts3d_h(0,1,2) = 0.0-eps;        //point near vertex (out)
        pts3d_h(0,2,0) = 1./3.-eps; pts3d_h(0,2,1) = 1./3.-eps; pts3d_h(0,2,2) = 1./3.-eps;  //point near face (in)
        pts3d_h(0,3,0) = 1./3.+eps; pts3d_h(0,3,1) = 1./3.+eps; pts3d_h(0,3,2) = 1./3.+eps;  //point near face (out)
        ...
        pts3d_h(0,0,0) = 0.0+eps; pts3d_h(0,0,1) = 0.0+eps; pts3d_h(0,0,2) = -1.0+eps;  //point near vertex (in)
        pts3d_h(0,1,0) = 0.0-eps; pts3d_h(0,1,1) = 0.0-eps; pts3d_h(0,1,2) = -1.0-eps;  //point near vertex (out)
        pts3d_h(0,2,0) = 0.5-eps; pts3d_h(0,2,1) = 0.5-eps; pts3d_h(0,2,2) = 0.0;       //point near face (in)
        pts3d_h(0,3,0) = 0.5+eps; pts3d_h(0,3,1) = 0.5+eps; pts3d_h(0,3,2) = 0.0;       //point near face (out)
        Kokkos::deep_copy(pts3d,pts3d_h);
        ...
        pts3d_h(0,0,0) = -1.0+eps; pts3d_h(0,0,1) = -1.0+eps; pts3d_h(0,0,2) = -1.0+eps;  //point near vertex (in)
        pts3d_h(0,1,0) = -1.0-eps; pts3d_h(0,1,1) = -1.0-eps; pts3d_h(0,1,2) = -1.0-eps;  //point near vertex (out)
        pts3d_h(0,2,0) = 0.0; pts3d_h(0,2,1) = 0.0; pts3d_h(0,2,2) = -1.0+eps;            //point near face (in)
        pts3d_h(0,3,0) = 0.0; pts3d_h(0,3,1) = 0.0; pts3d_h(0,3,2) = -1.0-eps;            //point near face (out)
        Kokkos::deep_copy(pts3d,pts3d_h);
