...
#define EXTRA_PRINT_FF_HELPER 0
                               ...
                               const BucketOrEntity& bucket_or_element, const MDArray& parametric_coordinates, double time_value_optional)
    {
      ...
      if (EXTRA_PRINT_FF_HELPER) 
        {
          std::vector<std::vector<double> > values(parametric_coordinates.extent(0));
          for(int i=0; i< values.size(); ++i) {
             values[i].resize(parametric_coordinates.extent(1));
             for(int j=0; j< values[i].size(); ++j)
               values[i][j] = parametric_coordinates(i,j);
          }             
      ...
      if (EXTRA_PRINT_FF_HELPER)
        {
          std::vector<std::vector<double> > values(basis_values.extent(0));
          for(int i=0; i< values.size(); ++i) {
             values[i].resize(basis_values.extent(1));
             for(int j=0; j< values[i].size(); ++j)
               values[i][j] = basis_values(i,j);
          }  
          std::cout << "FieldFunction::operator()(elem,...) basis_values = \n " << values << std::endl;
        }
      ...
      if (EXTRA_PRINT_FF_HELPER)
        {
          std::vector<std::vector<double> > values(transformed_basis_values.extent(0));
          for(int i=0; i< values.size(); ++i) {
             values[i].resize(transformed_basis_values.extent(1));
             for(int j=0; j< values[i].size(); ++j)
               values[i][j] = transformed_basis_values(i,j);
          }
          std::cout << "FieldFunction::operator()(elem,...) transformed_basis_values =  " << values << std::endl;
        }
