...
Piro::PerformAnalysis(
    ...
    const RCP< Thyra::VectorBase<double> > x_opt,
    const RCP< Thyra::VectorBase<double> > p_opt,
  ...
  if (analysis=="Solve") {
    ...
    *out << "Piro::PerformAnalysis: HDSA Post Optimality Analysis Being Performed " << endl;
    status = Piro::PerformHDSAAnalysis(piroModel,
                          piroParams, result, observer, x_opt, p_opt, x_diff_at_samples, p_samples);
 ...
 std::cout << __FILE__ << ": " << __LINE__ << ", norm(u_opt): "  << x_opt->norm_2() << std::endl;
    {
      ...
      if (!useIdentityStateEllOp && !model_PME.is_null()) {            
        ROL::ThyraVector<double> rol_opt_x(x_opt);
        model_PME->block_diagonal_hessian_22(H, *rol_x_ptr, rol_opt_x, state_ell_reponse_index,1);
      }
 ...
 std::cout << __FILE__ << ": " << __LINE__ << ", norm(u_opt): "  << x_opt->norm_2() << std::endl;
    {
      ...
      if (!useIdentityStateMassOp && !model_PME.is_null()) {
        ROL::ThyraVector<double> rol_opt_x(x_opt);
        model_PME->block_diagonal_hessian_22(H, *rol_x_ptr, rol_opt_x, state_mass_reponse_index,1);
        massOp = H;
      }
